...
from fairseq.models.lra.seqboat_lra_encoder import SeqBoatLRAEncoder
from fairseq.modules.seqboat_utils import Normal

def Linear(in_features, out_features, bias=True):
    m = nn.Linear(in_features, out_features, bias)
    #Normal(m.weight, 0, 0.02)
...
class LRAModel(FairseqEncoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--rel-pos-bias', default='simple')
        ...
        parser.add_argument('--act-bias', type=float, metavar='D',default = 0.0)
        parser.add_argument('--init-temp-scale', type=float, metavar='D',default = 0.3)
        parser.add_argument('--add-apos', default=False, action='store_true')
        parser.add_argument('--local-pos', default=False, action='store_true')
        parser.add_argument('--always-act', default=False, action='store_true')
        parser.add_argument('--use-nli', default=False, action='store_true')
    ...
    def forward(self, sample):
        ...
        loss = None
        if self.layer_type in ['slag']:
            loss = sentence_rep[2]
        if not self.use_p:
            if self.layer_type in ['transformer', 'lstm', 'flash', 'mega', 'slag']:
        ...
        if 'net_input1' in sample:
            ...
            if self.args.use_nli:
                concat_rep.append(sentence_rep*sentence1_rep)
                concat_rep.append(sentence_rep-sentence1_rep)
...
class LRAEncoder(FairseqEncoder):
    ...
    def __init__(self, args, task):
        ...
        elif args.layer_type == 'slag':
            self.encoder = SeqBoatLRAEncoder(
                padding_idx=padding_idx,
                vocab_size=vocab_size,
                num_encoder_layers=args.encoder_layers,
                embedding_type=embedding_type,
                ...
                max_steps= args.max_update,
                init_temp_scale = args.init_temp_scale,
                add_apos = args.add_apos,
                local_pos = args.local_pos,
                always_act = args.always_act,
            )
    ...
    def forward(self, src_tokens, src_lengths=None, **kwargs):    
        if self.args.layer_type == 'slag':
            return self.encoder(src_tokens, src_lengths, last_state_only=True, act_bias = self.args.act_bias)
        else:
...
@register_model_architecture('lra', 'slag_lra_listop')
def mega_lra_listop(args):
    rate = 1
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.layer_type = getattr(args, 'layer_type', 'slag')
    args.encoder_hidden_dim = getattr(args, 'encoder_hidden_dim', int(160*rate))
    args.z_dim = getattr(args, 'z_dim', int(64*rate))
    args.n_dim = getattr(args, 'n_dim', 16)
    args.encoder_layers = getattr(args, 'encoder_layers', 6)
    args.activation_fn = getattr(args, 'activation_fn', 'silu')
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', int(80*rate))
    args.classifier_layers = getattr(args, 'classifier_layers', 1)
    args.classifier_out_dim = getattr(args, 'classifier_out_dim', int(160*rate))
    args.chunk_size = getattr(args, 'chunk_size', -1)
    args.truncation_length = getattr(args, 'truncation_length', 1024)
    args.max_positions = getattr(args, 'max_positions', 2002)
    args.norm_type = getattr(args, 'norm_type', 'scalenorm')
...
@register_model_architecture('lra', 'slag_lra_listop_base')
def mega_lra_listop(args):
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.layer_type = getattr(args, 'layer_type', 'slag')
...
@register_model_architecture('lra', 'slag_lra_imdb')
def mega_lra_imdb(args):
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.layer_type = getattr(args, 'layer_type', 'slag')
    args.encoder_hidden_dim = getattr(args, 'encoder_hidden_dim', 256)
...
@register_model_architecture('lra', 'slag_lra_aan')
def mega_lra_aan(args):
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.layer_type = getattr(args, 'layer_type', 'slag')
    args.encoder_hidden_dim = getattr(args, 'encoder_hidden_dim', 256)
...
@register_model_architecture('lra', 'slag_lra_cifar10')
def mega_lra_cifar10(args):
    rate = 1
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.layer_type = getattr(args, 'layer_type', 'slag')
    args.encoder_hidden_dim = getattr(args, 'encoder_hidden_dim', int(320*rate))
    args.z_dim = getattr(args, 'z_dim', int(96*rate))
    args.n_dim = getattr(args, 'n_dim', 16)
    args.encoder_layers = getattr(args, 'encoder_layers', 8)
    args.activation_fn = getattr(args, 'activation_fn', 'silu')
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', int(160*rate))
    args.norm_type = getattr(args, 'norm_type', 'batchnorm')
    args.classifier_layers = getattr(args, 'classifier_layers', 1)
    args.classifier_out_dim = getattr(args, 'classifier_out_dim', int(320*rate))
    args.sentence_class_num = getattr(args, 'sentence_class_num', 10)
    args.chunk_size = getattr(args, 'chunk_size', 1024)
    args.truncation_length = getattr(args, 'truncation_length', 1024)
    args.max_positions = getattr(args, 'max_positions', 1024)
...
@register_model_architecture('lra', 'slag_lra_pf32')
def mega_lra_pf32(args):
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.layer_type = getattr(args, 'layer_type', 'slag')
    args.encoder_hidden_dim = getattr(args, 'encoder_hidden_dim', 256)
...
@register_model_architecture('lra', 'slag_lra_pf128')
def mega_lra_pf128(args):
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.layer_type = getattr(args, 'layer_type', 'slag')
    args.encoder_hidden_dim = getattr(args, 'encoder_hidden_dim', 128)
...
@register_model_architecture('lra', 'slag_lra_pf128_base')
def mega_lra_pf128(args):
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.layer_type = getattr(args, 'layer_type', 'slag')
    args.encoder_hidden_dim = getattr(args, 'encoder_hidden_dim', 256)
    ...
    args.sentence_class_num = getattr(args, 'sentence_class_num', 2)
    args.chunk_size = getattr(args, 'chunk_size', 128 * 128)
    args.truncation_length = getattr(args, 'truncation_length', 128 * 128)
    args.max_positions = getattr(args, 'max_positions', 128 * 128)
    args.sen_rep_type = getattr(args, 'sen_rep_type', 'mp')
    base_architecture(args)
