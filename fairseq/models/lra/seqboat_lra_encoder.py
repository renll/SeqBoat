...
class SeqBoatLRAEncoder(nn.Module):
    ...
    def __init__(
        ...
        if self.add_apos:
            self.pos_emb = ScaledSinuEmbedding(embedding_dim)
        ...
        self.layers.extend([
            self.build_slag_sentence_encoder_layer(
    ...
    def build_slag_sentence_encoder_layer(
        self,
        embedding_dim,
        hidden_dim,
        ffn_hidden_dim,
        ...
        return SeqBoatSentenceEncoderLayer(
    ...
    def forward(
        ...
        for i in range(self.num_layers):
            x, _ , loss = self.layers[i](x, x_padding_mask=padding_mask, act_bias=act_bias,layer_idx = i)
            if losses is None:
                losses = loss
            else:
                for j in range(len(losses)):
                    losses[j] = losses[j] + loss[j]
        ...
        if self.traceable:
            return torch.stack(inner_states), sentence_rep, losses
        else:
            return inner_states, sentence_rep, losses
...
class Embedding(nn.Module):
    ...
    def reset_parameters(self, embedding_dim):
        ...
        #Normal(self.embed.weight, mean=0, std=std)
