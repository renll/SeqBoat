...
from fairseq.modules.seqboat_utils import Normal, ScaledSinuEmbedding

@register_model("slag_lm")
class SeqBoatLanguageModel(FairseqLanguageModel):
    """
    Args:
    ...
    def add_args(parser):
        ...
        parser.add_argument('--hidden-dropout', type=float, metavar='D',
                            help='dropout probability for hidden vectors in Slag.')
        ...
        parser.add_argument('--normalize-embedding', action='store_true',
                            help='normalize embedding for Slag.')
    ...
    def build_model(cls, args, task):
        ...
        decoder = SeqBoatDecoderNoCrossAttn(args, src_dict, embed_tokens)
        ...
        if self.args.report_ema_alpha and report:
            with torch.no_grad():
                for lidx, layer in enumerate(self.decoder.layers):
                    # emb_dim x ndim
                    alpha = np.squeeze(layer.slag_layer.move.alpha.cpu().numpy(), axis=-1)
...
class SeqBoatDecoderNoCrossAttn(FairseqIncrementalDecoder):
    """
    Slag decoder consisting of *args.decoder_layers* layers. Each layer is a :class:`SlagDecoderLayer`.
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        #self.pos_emb = ScaledSinuEmbedding(embed_dim)
    ...
    def build_decoder_layer(self, args):
        return SeqBoatDecoderLayer(args, no_cross_attention=True)
...
@register_model_architecture("slag_lm", "slag_lm")
...
@register_model_architecture('slag_lm', 'slag_lm_big')
def slag_lm_big(args):
...
@register_model_architecture('slag_lm', 'slag_lm_wiki103')
@register_model_architecture('slag_lm', 'slag_lm_adaptive_big')
def slag_lm_adaptive_big(args):
...
@register_model_architecture('slag_lm', 'slag_lm_adaptive_base')
def slag_lm_adaptive_base(args):
    args.decoder_layers = getattr(args, 'decoder_layers', 16)
...
@register_model_architecture('slag_lm', 'slag_lm_enwik8_base')
def slag_lm_adaptive_big_enwik8(args):
...
@register_model_architecture('slag_lm', 'slag_lm_enwik8_base_v2')
def slag_lm_adaptive_big_enwik8(args):
    rate=1.2
    args.decoder_layers = getattr(args, 'decoder_layers', 14)
    args.decoder_embed_dim = getattr(args, "decoder_embed_dim", int(512*rate))
    args.decoder_hidden_dim = getattr(args, "decoder_hidden_dim", int(1024*rate))
    args.decoder_ffn_embed_dim = getattr(args, "decoder_ffn_embed_dim", int(1024*rate))
    args.decoder_z_dim = getattr(args, 'decoder_z_dim', 128)
    args.decoder_n_dim = getattr(args, 'decoder_n_dim', 16)
    args.dropout = getattr(args, 'dropout', 0.1)
    args.attention_dropout = getattr(args, 'attention_dropout', 0.0)
...
@register_model_architecture('slag_lm', 'slag_lm_enwik8_large')
def slag_lm_adaptive_large_enwik8(args):
