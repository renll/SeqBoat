...
@register_model("slag")
class SeqBoatModel(FairseqEncoderDecoderModel):
    """
    Args:
    ...
    def build_encoder(cls, args, src_dict, embed_tokens):
        return SeqBoatEncoder(args, src_dict, embed_tokens)

    @classmethod
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        return SeqBoatDecoder(args, tgt_dict, embed_tokens)
...
class SeqBoatEncoder(FairseqEncoder):
    """
    Slag encoder consisting of *args.encoder_layers* layers. Each layer is a :class:`SlagEncoderLayer`.
    ...
    def build_encoder_layer(self, args):
        return SeqBoatEncoderLayer(args)
...
class SeqBoatDecoder(FairseqIncrementalDecoder):
    """
    ...
    def build_decoder_layer(self, args):
        return SeqBoatDecoderLayer(args)
    ...
    def buffered_future_mask(self, tensor):
        ...
        if (
            ...
            if self.attention_activation == 'softmax':
                self._future_mask = torch.triu(torch.full((dim,dim),-10000),1)#utils.fill_with_neg_inf(torch.zeros(dim, dim)), 1)
...
@register_model_architecture("slag", "slag")
...
@register_model_architecture("slag", "slag_wmt_en_de")
def slag_wmt_en_de(args):
    args.dropout = getattr(args, "dropout", 0.1)
...
@register_model_architecture("slag", "slag_wmt_en_de_base")
def transformer_vaswani_wmt_en_de_base(args):
    ...
    args.encoder_z_dim = getattr(args, 'encoder_z_dim', 160)
    args.dropout = getattr(args, "dropout", 0.1)
    args.attention_dropout = getattr(args, "attention_dropout", 0.1)
    args.hidden_dropout = getattr(args, "hidden_dropout", 0.1)
    args.activation_dropout = getattr(args, "activation_dropout", 0.1)
...
@register_model_architecture("slag", "slag_wmt_en_de_big")
