...
from fairseq.modules.seqboat_unit import SeqBoatUnit
...
class SeqBoatEncoderLayer(nn.Module):
    """Encoder layer block.
    ...
    def __init__(self, args):
        ...
        self.slag_layer = self.build_slag_layer(self.embed_dim, args)
        if args.encoder_ffn_embed_dim > 0:
            self.nffn = self.build_nffn_layer(self.embed_dim, args)
        else:
            self.nffn = None

    def build_slag_layer(self, embed_dim, args):
    ...
    def forward(self, x, encoder_padding_mask):
        ...
        """
        x, _, _ = self.slag_layer(x, encoder_padding_mask)
...
class SeqBoatDecoderLayer(nn.Module):
    """Decoder layer block.
    ...
    def forward(
        ...
        Returns:
            encoded output of shape `(seq_len, batch, embed_dim)`
        """
        x, attn, _ = self.slag_layer(x=x, padding_mask=decoder_padding_mask,
